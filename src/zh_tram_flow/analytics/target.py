...
from zh_tram_flow.plot_styles import (style_ax, LEGEND_KW, LEGEND_KW_RIGHT, LEGEND_KW_LEFT,
                                               ...
                                               TITLE_KW, fmt_line_legend)
from zh_tram_flow.config import (BAR_NEUTRAL, auto_export, ARRIVAL_COLOR, DEPARTURE_COLOR, COMPARE_BASE,
                                  COMPARE_MODEL, ANNO_MEAN, ANNO_MEDIAN)
import pandas as pd
import numpy as np
...
def _draw_year_bars(ax, stats, cfg, style, with_trend=False):
    ...
    width  = 0.25
    _metric_colors = [ARRIVAL_COLOR, DEPARTURE_COLOR, cfg.COLOR_NEUTRAL]
    for i, (col, label, color) in enumerate(zip(
        ...
        bar_x = x_pos + i * width
        bar_label = None if with_trend else label   # Balken kein Label wenn Trends in Legende
        bars  = ax.bar(bar_x, vals, width, label=bar_label, color=color, alpha=0.85)
        for bar, v in zip(bars, vals):
            ax.text(bar.get_x() + bar.get_width() / 2,
                    v + (0.4 if v >= 0 else -2.0),
                    f"{v:+.1f}s", ha="center", va="bottom", fontsize=9)
        if with_trend and len(vals) > 1:
            ax.plot(bar_x, vals, **trend_kw(color, label=f"Trend {label}"))
    ax.set_xticks(x_pos + width)
    ax.set_xticklabels([str(y) for y in years], fontsize=11)
    ax.set_ylabel("Seconds", **style["label"])
    ax.set_ylim(bottom=None, top=90)
...
def plot_delay_overview_per_year(lf_all, lf_clean=None, cfg=None):
    ...
    if lf_clean is None:
        ...
        ax.set_title("Avg. Delay per Stop — by Year (raw)", **TITLE_KW)
    else:
        ...
        ax1.set_title("Raw Data", **TITLE_KW)
        ax2.set_title("Cleaned Data with Trend", **TITLE_KW)
...
def plot_monthly_delay(lf_all, lf_clean=None, cfg=None):
    ...
    style   = mpl_style()
    _trend_metrics = [
        ("arr_mean",   "Arrival Delay",   ARRIVAL_COLOR),
        ("dep_mean",   "Departure Delay", DEPARTURE_COLOR),
    ]
    _all_metrics = _trend_metrics + [("delta_mean", "Delay Delta", cfg.COLOR_NEUTRAL)]
    metrics = _all_metrics

    def _draw_monthly(ax, df, title, with_trend=False, legend_data=True, legend_trend=False):
        x = np.arange(len(df))
        data_handles, data_labels = [], []
        trend_handles, trend_labels = [], []
        for col, label, color in metrics:
            y = df[col].values
            line, = ax.plot(df["date"], y, **data_line_kw(color, label))
            data_handles.append(line); data_labels.append(label)
        # Trend only for arrival + departure (not delay delta)
        if with_trend:
            for col, label, color in _trend_metrics:
                y = df[col].values
                if len(y) > 1:
                    coeffs = np.polyfit(np.arange(len(y)), y, 1)
                    tline, = ax.plot(df["date"], np.polyval(coeffs, np.arange(len(y))), **trend_kw(color, f"Trend {label}"))
                    trend_handles.append(tline); trend_labels.append(f"Trend {label}")
        for year in [2024, 2025]:
            ax.axvline(pd.Timestamp(f"{year}-01-01"), color=cfg.CHART_AXIS, lw=1.0, linestyle=":")
        ax.set_ylabel("Avg. Seconds", **style["label"])
        ax.set_ylim(top=100)
        ax.set_title(title, **TITLE_KW)
        if legend_data and not legend_trend:
            ax.legend(data_handles, data_labels, **LEGEND_KW_RIGHT)
        elif legend_trend:
            ax.legend(trend_handles, trend_labels, **LEGEND_KW_RIGHT)
        style_ax(ax)

    if lf_clean is None:
        df_all = _compute_monthly(lf_all)
        fig, ax = plt.subplots(figsize=(14, 5))
        _draw_monthly(ax, df_all, "Raw Data", legend_data=True)
    else:
        df_all   = _compute_monthly(lf_all)
        df_clean = _compute_monthly(lf_clean)
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 5), sharey=False)
        _draw_monthly(ax1, df_all,   "Raw Data",                           legend_data=True)
        _draw_monthly(ax2, df_clean, "Cleaned Data with Trend",            with_trend=True, legend_data=False, legend_trend=True)
...
def plot_delay_distribution(lf, cfg=None):
    ...
    _hist_colors = [ARRIVAL_COLOR, DEPARTURE_COLOR, cfg.COLOR_NEUTRAL]
    CLIP = (-300, 600)

    fig, axes = plt.subplots(1, 3, figsize=(16, 4))
    for ax, col, title, color in zip(axes, delay_cols, titles, _hist_colors):
    ...
    plt.suptitle("Distribution — 100k Sample (clipped −300 to +600s)", fontsize=11, color=cfg.CHART_TITLE, y=1.01)
    plt.tight_layout()
    plt.show()
...
def plot_delay_distribution_comparison(lf_all, lf_clean=None, cfg=None):
    ...
    style = mpl_style()
    # Jede Spalte = Metrik-Farbe; Zeile 0 (roh) = alpha 0.45, Zeile 1 (bereinigt) = alpha 0.85
    metrics = [
        ("arrival_delay",   "Arrival Delay",   ARRIVAL_COLOR,   (-200, 600)),
        ("departure_delay", "Departure Delay", DEPARTURE_COLOR, (-200, 600)),
        ("delay_delta",     "Delay Delta",     cfg.COLOR_NEUTRAL, (-150, 150)),
    ]
    _row_alpha = [0.45, 0.85]

    fig, axes = plt.subplots(2, 3, figsize=(18, 8), sharey=False)
    plt.subplots_adjust(hspace=0.45)

    _row_suffix = ["Raw Data", "Cleaned Data"]

    for col_i, (col, label, color, clip) in enumerate(metrics):
        data_all = df_all[col].clip(*clip)
        data_clean = df_clean[col].clip(*clip)
        bins = np.linspace(clip[0], clip[1], 80)

        for row_i, (data, suffix) in enumerate([
            (data_all,   _row_suffix[0]),
            (data_clean, _row_suffix[1]),
        ]):
            ax = axes[row_i][col_i]
...
def plot_log_transform(lf, cfg=None):
    ...
    ax2.set_ylabel("Count", **style["label"])
    ax2.set_xlim(-5, 11)
    ax2.set_ylim(0, 10000)
    ax2.legend(**LEGEND_KW_RIGHT)
    style_ax(ax2)

    plt.suptitle("Log-Transform compresses extremes — distribution approaches normal form",
                 fontsize=14, color=cfg.CHART_TITLE, x=0.02, ha="left", y=1.02)
    plt.tight_layout()
    plt.show()
...
def plot_arrival_vs_departure(lf, cfg=None, ylim=None):
    ...
    style = mpl_style()
    _box_colors = [ARRIVAL_COLOR, DEPARTURE_COLOR, cfg.COLOR_NEUTRAL]
    labels = ["Arrival\nDelay", "Departure\nDelay", "Delay\nDelta"]
    ...
    bp = ax.boxplot(
        ...
        labels=labels,
        patch_artist=True,
        medianprops=dict(color=ANNO_MEDIAN, linewidth=1.0, linestyle="--"),
...
def plot_start_stop_analysis(lf_delay, cfg=None):
    ...
    ax.hist(df_ns["delay_delta"].clip(*CLIP), bins=bins, density=True,
            alpha=0.55, color=COMPARE_BASE, label="Regular Stops (seq > 1)")
    ax.hist(df_s["delay_delta"].clip(*CLIP), bins=bins, density=True,
            alpha=0.65, color=ARRIVAL_COLOR, label="First Stops (seq == 1)")
    ...
    for b, v in zip(list(b1) + list(b2), arr_vals + delta_vals):
        ax2.text(b.get_x() + b.get_width() / 2, v + (0.5 if v >= 0 else -2.5),
                 f"{v:+.1f}s", ha="center", va="bottom", fontsize=9)
    ax2.set_xticks(x)
    ax2.set_xticklabels(_bar_labels, fontsize=10)
    ax2.set_ylabel("Seconds", **style["label"])
    ax2.set_title("Avg. Delay Values", **TITLE_KW)
    ax2.set_ylim(bottom=-0.5, top=80)
    ax2.legend(**LEGEND_KW_RIGHT)
    style_ax(ax2)

    ax3 = axes[2]
    pct_vals = [row_s["pct_neg_delta"] * 100, row_ns["pct_neg_delta"] * 100]
    bar_colors = [cfg.COLOR_NEGATIVE, ARRIVAL_COLOR]
    bars = ax3.bar(_bar_labels, pct_vals, color=bar_colors, alpha=0.85)
    for b, v in zip(bars, pct_vals):
        ax3.text(b.get_x() + b.get_width() / 2, v + 0.5, f"{v:.1f}%",
                 ha="center", fontsize=9)
    ax3.set_ylabel("Share Negative Delay Delta (%)", **style["label"])
    ax3.set_title("Early Arrivals / Early Departures", **TITLE_KW)
    ax3.set_ylim(0, 100)
...
def plot_otp(lf, cfg=None, ylim_otp=None, ylim_delta=None):
    ...
    b1 = ax1.bar(x - width / 2, arr_vals, width, label="Arrival", color=ARRIVAL_COLOR)
    b2 = ax1.bar(x + width / 2, dep_vals, width, label="Departure", color=DEPARTURE_COLOR)
    ax1.set_xticks(x)
    ax1.set_xticklabels(categories, fontsize=10)
    ax1.set_ylabel("Share", **style["label"])
    ax1.set_title("Arrival vs Departure — OTP Overview", **TITLE_KW)
...
def plot_otp_per_line(lf_all, cfg=None, ylim_otp=(20, 120)):
    ...
    ax.set_ylim(ylim_otp[0] / 100, ylim_otp[1] / 100)
...
def table_cancellations_by_line(lf):
    ...
    return (
        ...
        .sort("cancel_rate", descending=False)
        .head(15)
        .collect()
        .to_pandas()
        .rename(columns={"line_name": "Linie", "total": "Gesamt",
                         "canceled_count": "Ausgefallen", "cancel_rate": "Cancellation Rate"})
        .assign(Ausfallrate=lambda df: df["Cancellation Rate"].apply(lambda x: f"{x:.1%}"))
...
def plot_cancellations_by_line(lf, cfg=None):
    ...
    bar_colors = [line_color(str(ln)) for ln in cancel_by_line["line_name"]]

    fig, ax = plt.subplots(figsize=(12, 5))
    bars = ax.barh(cancel_by_line["line_name"].astype(str), cancel_by_line["cancel_rate"], color=bar_colors)
    ...
    ax.set_title("Cancellation Rate — Top 15 Lines", **TITLE_KW)
    ax.legend(**LEGEND_KW_RIGHT)
    style_ax(ax)
    plt.tight_layout()
    plt.show()
...
def plot_trip_level_validation(master_path, cfg=None):
    ...
    trip_cancel = (
        ...
        .with_columns([
            pl.when(pl.col("cancel_share") == 1.0).then(pl.lit("canceled"))
              .when(pl.col("cancel_share") == 0.0).then(pl.lit("active"))
    ...
    summary = (
        ...
        .with_columns(
            pl.when(pl.col("is_pre_july_2024"))
              ...
              .otherwise(pl.lit("from Jul 2024"))
    ...
    n_canceled = trip_cancel.filter(pl.col("trip_type") == "canceled").height
    ...
    style  = mpl_style()
    colors = {
        "active":   cfg.COLOR_POSITIVE,
        "mixed":          BAR_NEUTRAL,
        "canceled": cfg.COLOR_NEGATIVE,
    }
    periods = ["pre Jul 2024", "from Jul 2024"]
    types   = ["active", "mixed", "canceled"]
    ...
    for ax, normalize in zip(axes, [False, True]):
        ...
        _title = "Share of Trips by Type" if normalize else "Number of Trips by Type"
        ax.set_title(_title, **TITLE_KW)
        if normalize:
            ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda v, _: f"{v:.0%}"))
            ax.set_ylim(0, 1.2)
        else:
...
def plot_cancellation_rate_over_time(lf_all, cfg=None, ylim=None):
    ...
    ax.axvspan(pd.Timestamp("2023-01-01"), pd.Timestamp("2024-06-30"),
               ...
               label="Glattalbahn Construction (Jan 2023 – Jun 2024)")
    for year in [2024, 2025]:
        ax.axvline(pd.Timestamp(f"{year}-01-01"), color=cfg.CHART_AXIS, lw=1, linestyle=":")
...
def plot_delay_per_line_timeline(lf_all, cfg=None,
                                  ylim_arr=(20, 200), ylim_dep=(20, 180), ylim_delta=(0, 40)):
    ...
    lines_all = sorted(delay_monthly_line["line_name"].astype(str).unique(),
                       key=lambda x: int(x) if x.isdigit() else 99)
    metrics = [
        ("arr_mean",   "Arrival Delay",   ylim_arr),
        ("dep_mean",   "Departure Delay", ylim_dep),
        ("delta_mean", "Delay Delta",     ylim_delta),
    ]
