...
### 2026-06-16 — Notebook-Struktur vollständig überarbeitet

**Was wurde gemacht:**

- **Externe Review `00_introduction.ipynb`** — Feedback-Liste aus Perspektive Tech Lead / HR Recruiter: 16 Punkte (Zeilenzahlen-Inkonsistenz, `prev_trip_delay` Operationalisierung, Berlin-Referenz, fehlende Deliverables etc.)
- **`00_introduction.ipynb`** überarbeitet:
  - ToC eingefügt, Berlin aus Mission entfernt, MIV als Einflussfaktor ergänzt
  - Netzstruktur: Anbindungsveränderungen (K3/K8/K9) + VBZ-Betriebsqualität als Befund
  - Neuer Abschnitt "Ergebnisse & Deliverables" (7 Artefakte mit Links)
  - Data Dictionary → Link auf `docs/DATA_DICTIONARY.md` (Redundanz aufgelöst)
  - Workflow → Projekt-Lifecycle-Tabelle (6 Phasen)
- **`04_insights.ipynb`** — ToC + "Zentrale Fragen → Befunde"-Tabelle nach Kernthese
- **Alle 20 Notebooks** — einheitliches Titelformat: `# Title (English)` / `**UPPERCASE SUBTITLE**` / `---`
- **ToC in 4 Notebooks** eingefügt: 01_exploration, 02_preparation, 03_analysis_0-overview, 06_prediction_7
- **`docs/CONVENTIONS.md`** — Header-Zellen-Konvention auf alle Ebenen `# – ####` erweitert
- **`03_analysis_0-overview.ipynb`** vollständig reviewed:
  - Hierarchiefehler behoben: `##` → `###` für alle Unterabschnitte von Analysis Result
  - Alle deutschen Section-Header ins Englische übersetzt (Q1–Q6, Data Strategy, etc.)
  - F-NET-07: `open` → `done` (prev_trip_delay implementiert, stärkstes Feature in v2)
  - Kernfragen & KPIs aktualisiert: Modell-Ergebnisse eingetragen, alle `🔜` aufgelöst
  - Findings-Register in `03_analysis_7-findings.ipynb` ausgelagert (63 F-IDs)
  - Modelling Insights vollständig neu geschrieben: professionelles Englisch, Past Tense, bestätigte Ergebnisse statt Planung
- **`03_analysis_7-findings.ipynb`** neu erstellt — dediziertes Findings-Register
- **`03_analysis_0-overview` Sections umbenannt**: Key Findings → Findings, Report-Auswahl → Key Findings

**Entscheidungen:**

- Data Dictionary: `docs/DATA_DICTIONARY.md` ist Single Source of Truth — Notebook-Tabelle war redundant und hatte Inkonsistenzen (94M vs. 94.4M)
- Findings auslagern: 63 F-IDs im Overview waren überwältigend für externe Leser — Key Findings bleibt, vollständiges Register in eigenem Notebook
- Workflow-Tabelle: Lifecycle-Format (DAN + DSC kombiniert) statt flacher Notebook-Liste (die ist in der README)
