...
| # | Beschreibung | Prio |
| :--- | :--- | :--- |
| 36 | **Narrative-These: "Drei Findings — eine Aussage"** — `dwell_time = 0s` + Peripherie-Hotspots + Netzausbau am falschen Ort als explizite Synthese-These formulieren: *"Die Verspätungen sind vorhersagbar, systematisch — und das Geld wurde trotzdem am falschen Ort investiert."* Prominent in `03_analysis_0-overview.ipynb` (Executive Summary) + `04_insights.ipynb` (Einstieg) einbauen. Aktuell: drei separate Findings, keine Synthese. | 1 |
| 37 | **dwell_time als Root Cause framen** — In `03_analysis_1-target.ipynb` eine eigene Sektion "Root Cause" einbauen mit dem Statement: *"Das Verspätungsproblem ist nicht operativ — es ist im Fahrplan eingebaut."* 71.3% dwell_time = 0s ist keine Finding-Zeile, das ist eine Systemdiagnose. Aktuell: Finding vorhanden, aber als Nebenbefund ohne Gewicht. | 1 |
